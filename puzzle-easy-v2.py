...
layout = [
    ...
    [
        ...
        [sg.Button('Restart', key="Restart", font=('Kohinoor Telugu', 10))
         ],
...
while True:
    while True:
        ...
        if event in (sg.WINDOW_CLOSED, 'Exit'):
            break
        # ... [your button event handlers remain unchanged]
        
        elif event == 'Restart':
            numbers = [0,0,0,0,0]
            
            for idx in range(5):
                window[f'Number{idx}'].update(numbers[idx])

            # Optionally: Reconnect to the server
            client_socket.close()
            client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            client_socket.connect((server_ip, server_port))
